    ...
    <>
      ...
      >
        ...
        <div className="px-4 pt-4 pb-2">
          <button
            onClick={handleNew}
            className="w-full flex items-center gap-2 bg-blue-600 hover:bg-blue-500 text-white rounded-xl px-4 py-2.5 text-sm font-semibold transition-colors shadow-sm"
          >
              ...
              {renameId === conv.id ? (
                ...
                <div
                  ...
                  className={`flex items-center justify-between gap-2 rounded-xl px-3 py-2.5 cursor-pointer transition-colors group
                    ${activeId === conv.id ? "bg-blue-50 border border-blue-200" : "hover:bg-slate-100"}`}
                >
                  <div className="flex items-center gap-2 flex-1 min-w-0">
                    {conv.isFavorite && <span className="text-yellow-500 text-xs flex-shrink-0">★</span>}
                    <span className={`text-sm truncate ${activeId === conv.id ? "text-blue-800 font-medium" : "text-slate-600"}`}>
                      {conv.title}
                    </span>
                  </div>
                  <button
